...
void PostProcessSample::Compositor::blit(const VkRectangle& dst)
{
...
{
	VkRectangle defaultViewport = Game::getInstance()->getViewport();
    
    // Draw into the framebuffer
    Game::getInstance()->setViewport(VkRectangle(FRAMEBUFFER_WIDTH, FRAMEBUFFER_HEIGHT));
	...
	VkRectangle offsetViewport = VkRectangle(getWidth() - quarterWidth, 0, quarterWidth, quarterHeight);
...
void PostProcessSample::drawTechniqueId(const char* techniqueId)
{
    ...
    _font->start();
    _font->drawText(buffer, VkRectangle(0, 10, getWidth(), getHeight()), Vector4::one(), 18, Font::ALIGN_TOP_HCENTER);
    _font->finish();
}
