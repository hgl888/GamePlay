...
void WaterSample::render(float elapsedTime)
{
	...
	VkRectangle defaultViewport = getViewport();
    setViewport(VkRectangle(BUFFER_SIZE, BUFFER_SIZE));
    _clipPlane.y = -1.f;
    ...
    {
        _refractBatch->start();
        _refractBatch->draw(Vector3(0.f, 4.f, 0.f), VkRectangle(BUFFER_SIZE, BUFFER_SIZE), Vector2(426.f, 240.f));
        _refractBatch->finish();
        _reflectBatch->start();
        _reflectBatch->draw(Vector3(430.f, 4.f, 0.f), VkRectangle(BUFFER_SIZE, BUFFER_SIZE), Vector2(426.f, 240.f));
