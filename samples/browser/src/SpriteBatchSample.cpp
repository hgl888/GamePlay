...
void SpriteBatchSample::render(float elapsedTime)
{
	...
	VkRectangle dst(0, 0, 64, 64);
	VkRectangle src(0, 0, 256, 256);
    ...
    // Color tint
    _spriteBatch->draw(VkRectangle( 64, 0, 64, 64), src, Vector4::fromColor(0xF68B28FF));
    _spriteBatch->draw(VkRectangle(128, 0, 64, 64), src, Vector4::fromColor(0xDA2128FF));
    _spriteBatch->draw(VkRectangle(192, 0, 64, 64), src, Vector4::fromColor(0xE21B52FF));
    _spriteBatch->draw(VkRectangle(256, 0, 64, 64), src, Vector4::fromColor(0xE12991FF));
    _spriteBatch->draw(VkRectangle(320, 0, 64, 64), src, Vector4::fromColor(0x9A258FFF));
    _spriteBatch->draw(VkRectangle(384, 0, 64, 64), src, Vector4::fromColor(0x4D3F99FF));
    _spriteBatch->draw(VkRectangle(448, 0, 64, 64), src, Vector4::fromColor(0x0073BCFF));
    _spriteBatch->draw(VkRectangle(512, 0, 64, 64), src, Vector4::fromColor(0x00A8DFFF));
    _spriteBatch->draw(VkRectangle(576, 0, 64, 64), src, Vector4::fromColor(0x00AFADFF));
    _spriteBatch->draw(VkRectangle(640, 0, 64, 64), src, Vector4::fromColor(0x00A95CFF));
    _spriteBatch->draw(VkRectangle(704, 0, 64, 64), src, Vector4::fromColor(0x8CC747FF));
    _spriteBatch->draw(VkRectangle(768, 0, 64, 64), src, Vector4::fromColor(0xFFE710FF));

    // Negative height draw over top of the first one
    _spriteBatch->draw(VkRectangle(0, 0 , 64 * 2.0f, 64 * -2.0f), src);
    ...
    for (unsigned int i = 0; i < pointCount; i++)
    {
        ...
        {
            _spriteBatch->draw(VkRectangle(x, y, 32, 32), src);
            x += 32;    
        }
        x = 0;
        y += 32;
    ...
    // 50% transparent
    _spriteBatch->draw(VkRectangle(x + 512, y - 512, 512, 512), src, Vector4(1, 1, 1, 0.5f));
    _spriteBatch->finish();
