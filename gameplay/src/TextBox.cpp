...
unsigned int TextBox::drawImages(Form* form, const VkRectangle& clip)
{
    ...
    if (_caretImage && (state == ACTIVE || hasFocus()))
    {
        ...
        const VkRectangle& region = _caretImage->getRegion();
        if (!region.isEmpty())
        {
...
unsigned int TextBox::drawText(Form* form, const VkRectangle& clip)
{
    if (_text.size() <= 0)
        return 0;
