...
public:
    ...
    static Sprite* create(const char* imagePath, float width, float height,
                          const VkRectangle& source, unsigned int frameCount = 1,
                          Effect* effect = NULL);
     ...
     */
    void setFrameSource(unsigned int frameIndex, const VkRectangle& source);
     ...
     */
    const VkRectangle& getFrameSource(unsigned int frameIndex) const;
