...
void Font::lazyStart()
{
    ...
    const VkRectangle& vp = Game::getInstance()->getViewport();
    if (!vp.isEmpty())
    {
...
void Font::drawText(const char* text, const VkRectangle& area, const Vector4& color, unsigned int size, Justify justify, bool wrap, bool rightToLeft, const VkRectangle& clip)
{
    GP_ASSERT(text);
    GP_ASSERT(_size);
...
void Font::measureText(const char* text, const VkRectangle& clip, unsigned int size, VkRectangle* out, Justify justify, bool wrap, bool ignoreClip)
{
    GP_ASSERT(_size);
    GP_ASSERT(text);
    GP_ASSERT(out);
...
void Font::getMeasurementInfo(const char* text, const VkRectangle& area, unsigned int size, Justify justify, bool wrap, bool rightToLeft,
...
int Font::getIndexAtLocation(const char* text, const VkRectangle& area, unsigned int size, const Vector2& inLocation, Vector2* outLocation,
...
void Font::getLocationAtIndex(const char* text, const VkRectangle& clip, unsigned int size, Vector2* outLocation, const unsigned int destIndex,
                              Justify justify, bool wrap, bool rightToLeft)
{
    getIndexOrLocation(text, clip, size, *outLocation, outLocation, (const int)destIndex, justify, wrap, rightToLeft);
}

int Font::getIndexOrLocation(const char* text, const VkRectangle& area, unsigned int size, const Vector2& inLocation, Vector2* outLocation,
...
void Font::addLineInfo(const VkRectangle& area, int lineWidth, int lineLength, Justify hAlign,
