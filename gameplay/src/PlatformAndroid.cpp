...
static int32_t engine_handle_input(struct android_app* app, AInputEvent* event)
{
    ...
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION)
    {
        ...
        if ((source & AINPUT_SOURCE_JOYSTICK) == AINPUT_SOURCE_JOYSTICK)
        {
            ...
            if (xaxis == -1.0f)
            {
                vk::Platform::gamepadButtonPressedEventInternal(deviceId, vk::Gamepad::BUTTON_LEFT);
            }
            else if(xaxis == 1.0f)
            {
                vk::Platform::gamepadButtonPressedEventInternal(deviceId, vk::Gamepad::BUTTON_RIGHT);
            }
            else if (xaxis == 0.0f)
            {
                vk::Platform::gamepadButtonReleasedEventInternal(deviceId, vk::Gamepad::BUTTON_LEFT);
                vk::Platform::gamepadButtonReleasedEventInternal(deviceId, vk::Gamepad::BUTTON_RIGHT);
            }

            if(yaxis == -1.0f)
            {
                vk::Platform::gamepadButtonPressedEventInternal(deviceId, vk::Gamepad::BUTTON_UP);
            }
            else if(yaxis == 1.0f)
            {
                vk::Platform::gamepadButtonPressedEventInternal(deviceId, vk::Gamepad::BUTTON_DOWN);
            }
            else if (yaxis == 0.0f)
            {
                vk::Platform::gamepadButtonReleasedEventInternal(deviceId, vk::Gamepad::BUTTON_UP);
                vk::Platform::gamepadButtonReleasedEventInternal(deviceId, vk::Gamepad::BUTTON_DOWN);
            }

            // Trigger handling
            float leftTrigger = AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_BRAKE, 0);
            vk::Platform::gamepadTriggerChangedEventInternal(deviceId, 0, leftTrigger);
            float rightTrigger = AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_GAS, 0);
            vk::Platform::gamepadTriggerChangedEventInternal(deviceId, 1, rightTrigger);

            // Joystick handling
            float fuzz = 0.15f;
            float x = AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_X, 0);
            float y = -AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_Y, 0);
            vk::Platform::gamepadJoystickChangedEventInternal(deviceId, 0, clampFuzz(x, fuzz), clampFuzz(y, fuzz));
            ...
            vk::Platform::gamepadJoystickChangedEventInternal(deviceId, 1, clampFuzz(z, fuzz), clampFuzz(rz, fuzz));
        }
        else
        {
                ...
                case AMOTION_EVENT_ACTION_DOWN:
                    {
                        ...
                        vk::Platform::touchEventInternal(Touch::TOUCH_PRESS, x, y, pointerId);
                ...
                case AMOTION_EVENT_ACTION_UP:
                    {
					    ...
					    if (__pointer0.pressed &&  __pointer0.pointerId == pointerId && !Form::getActiveControl())
                        {
						    ...
						    else if (__gestureDraging)
                            {
                                ...
                                {
                                    vk::Platform::gestureDropEventInternal(x, y);
                                    gestureDetected = true;
                                }
                            ...
                            // Test for swipe
                                ...
                                vk::Game::getInstance()->getAbsoluteTime() - __pointer0.time < GESTURE_SWIPE_DURATION_MAX && 
                                (abs(deltaX) > GESTURE_SWIPE_DISTANCE_MIN || abs(deltaY) > GESTURE_SWIPE_DISTANCE_MIN) )
                            {
                                ...
                                {
                                    if (deltaX > 0)
                                        direction = vk::Gesture::SWIPE_DIRECTION_RIGHT;
                                    else if (deltaX < 0)
                                        direction = vk::Gesture::SWIPE_DIRECTION_LEFT;
                                }
                                else
                                {
                                    if (deltaY > 0)
                                        direction = vk::Gesture::SWIPE_DIRECTION_DOWN;
                                    else if (deltaY < 0)
                                        direction = vk::Gesture::SWIPE_DIRECTION_UP;
                                }
                                vk::Platform::gestureSwipeEventInternal(x, y, direction);
                                gestureDetected = true;
                            }
                            // Test for tap
                            else if(__gestureEventsProcessed.test(Gesture::GESTURE_TAP) &&
                                   vk::Game::getInstance()->getAbsoluteTime() - __pointer0.time < GESTURE_TAP_DURATION_MAX)
                            {
                                vk::Platform::gestureTapEventInternal(x, y);
                                gestureDetected = true;
                            }
                            // Test for long tap
                            else if(__gestureEventsProcessed.test(Gesture::GESTURE_LONG_TAP) &&
                                   vk::Game::getInstance()->getAbsoluteTime() - __pointer0.time >= GESTURE_LONG_TAP_DURATION_MIN)
                            {
                                vk::Platform::gestureLongTapEventInternal(x, y, vk::Game::getInstance()->getAbsoluteTime() - __pointer0.time);
                                gestureDetected = true;
                            }    
                        }
                        ...
                        {
                            vk::Platform::touchEventInternal(Touch::TOUCH_RELEASE, x, y, pointerId);
                        }
                        __primaryTouchId = -1;
                    }
                    break;
                ...
                case AMOTION_EVENT_ACTION_POINTER_DOWN:
                    {
                        ...
                        if (__multiTouch)
                        {
                            vk::Platform::touchEventInternal(Touch::TOUCH_PRESS, 
                ...
                case AMOTION_EVENT_ACTION_POINTER_UP:
                    {
                        ...
                        if (__pointer1.pressed &&  __pointer1.pointerId == pointerId && !Form::getActiveControl())
                        {
						    ...
						    // Test for swipe
                                ...
                                vk::Game::getInstance()->getAbsoluteTime() - __pointer1.time < GESTURE_SWIPE_DURATION_MAX && 
                                (abs(deltaX) > GESTURE_SWIPE_DISTANCE_MIN || abs(deltaY) > GESTURE_SWIPE_DISTANCE_MIN) )
                            {
                                int direction = 0;
                                if (deltaX > 0)
                                ...
                                gestureDetected = true;
                                   ...
                                   vk::Game::getInstance()->getAbsoluteTime() - __pointer1.time < GESTURE_TAP_DURATION_MAX)
                            {
                                vk::Platform::gestureTapEventInternal(x, y);
                                gestureDetected = true;
                            }
                            else if(__gestureEventsProcessed.test(Gesture::GESTURE_LONG_TAP) &&
                                   vk::Game::getInstance()->getAbsoluteTime() - __pointer1.time >= GESTURE_LONG_TAP_DURATION_MIN)
                            {
                                vk::Platform::gestureLongTapEventInternal(x, y, vk::Game::getInstance()->getAbsoluteTime() - __pointer1.time);
                                gestureDetected = true;
                            }    
                        }
                        ...
                        if (!gestureDetected && (__multiTouch || __primaryTouchId == pointerId) )
                        {
                            vk::Platform::touchEventInternal(Touch::TOUCH_RELEASE, 
                                                                   AMotionEvent_getX(event, pointerIndex), 
                                                                   AMotionEvent_getY(event, pointerIndex), pointerId);
                        }
                ...
                case AMOTION_EVENT_ACTION_MOVE:
                    {
                        ...
                        for (size_t i = 0; i < pointerCount; ++i)
                        {
							    ...
							    if (__pointer1.pressed && (pointerId == __pointer0.pointerId || pointerId == __pointer1.pointerId))
							    {
								    ...
								    if (__gestureEventsProcessed.test(Gesture::GESTURE_PINCH))
								    {
									    ...
									    if (__gesturePinching)
									    {
										    ...
										    {
											    vk::Platform::gesturePinchEventInternal(__gesturePinchCentroid.first, __gesturePinchCentroid.second, scale);	
											    gestureDetected = true;
										    }
										    else
											    __gesturePinching = false;
							    ...
							    else if (!gestureDetected && pointerId == __pointer0.pointerId)
							    {
								    ...
								    if (__gestureEventsProcessed.test(Gesture::GESTURE_DRAG))
								    {
                                		...
                                		if ((__gestureDraging || __gestureEventsProcessed.test(Gesture::GESTURE_DRAG)) &&
                                     		(vk::Game::getInstance()->getAbsoluteTime() - __pointer0.time >= GESTURE_DRAG_START_DURATION_MIN) &&
                                    		(delta >= GESTURE_DRAG_DISTANCE_MIN))
                                		{
                                    		vk::Platform::gestureDragEventInternal(x, y);
                            ...
                            if (!gestureDetected && (__multiTouch || __primaryTouchId == pointerId))
                            {
                                vk::Platform::touchEventInternal(Touch::TOUCH_MOVE, AMotionEvent_getX(event, i), AMotionEvent_getY(event, i), pointerId);
                            }
                       }
                   }
                   break;
    ...
    else if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_KEY)
    {
        ...
        switch(action)
        {
            case AKEY_EVENT_ACTION_DOWN:
                ...
                {
                    vk::Platform::gamepadButtonPressedEventInternal(deviceId, vk::getGamepadButtonMapping(keycode));
                }
                else
                {
                    vk::Platform::keyEventInternal(Keyboard::KEY_PRESS, getKey(keycode, metastate));
                    if (int character = getUnicode(keycode, metastate))
                        vk::Platform::keyEventInternal(Keyboard::KEY_CHAR, character);
                }
                break;
            ...
            case AKEY_EVENT_ACTION_UP:
                if (((source & AINPUT_SOURCE_GAMEPAD) == AINPUT_SOURCE_GAMEPAD) || ((source & AINPUT_SOURCE_JOYSTICK) == AINPUT_SOURCE_JOYSTICK))
                {
                    vk::Platform::gamepadButtonReleasedEventInternal(deviceId, vk::getGamepadButtonMapping(keycode));
                }
                else
                {
                    vk::Platform::keyEventInternal(Keyboard::KEY_RELEASE, getKey(keycode, metastate));
                }
                break;
        }
        return 1;
...
int Platform::enterMessagePump()
{
    ...
    while (true)
    {
        ...
        vk::displayKeyboard(__state, __displayKeyboard);
    }
    return 0;
}
...
bool Platform::isGestureSupported(Gesture::GestureEvent evt)
{
    ...
    return evt == vk::Gesture::GESTURE_SWIPE || evt == vk::Gesture::GESTURE_TAP || evt == vk::Gesture::GESTURE_LONG_TAP ||
        evt == vk::Gesture::GESTURE_DRAG || evt == vk::Gesture::GESTURE_DROP || evt == vk::Gesture::GESTURE_PINCH;
}
...
JNIEXPORT void JNICALL Java_org_gameplay3d_GamePlayNativeActivity_gamepadEventConnectedImpl(JNIEnv* env, jclass clazz, jint deviceId, jint buttonCount, jint joystickCount, jint triggerCount, jstring deviceName)
{
	...
	vk::Platform::gamepadEventConnectedInternal(deviceId, buttonCount, joystickCount, triggerCount, name);
...
{
	vk::Platform::gamepadEventDisconnectedInternal(deviceId);
}
