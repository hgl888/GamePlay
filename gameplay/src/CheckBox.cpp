...
void CheckBox::updateBounds()
{
    ...
    if (_checked)
    {
        const VkRectangle& selectedRegion = getImageRegion("checked", NORMAL);
        size.set(selectedRegion.width, selectedRegion.height);
    }
    else
    {
        const VkRectangle& unselectedRegion = getImageRegion("unchecked", NORMAL);
...
unsigned int CheckBox::drawImages(Form* form, const VkRectangle& clip)
{
    if (!_image)
        return 0;
