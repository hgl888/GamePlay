...
Container::Container()
    : _layout(NULL), _activeControl(NULL), _scrollBarTopCap(NULL), _scrollBarVertical(NULL), _scrollBarBottomCap(NULL),
      ...
      _scroll(SCROLL_NONE), _scrollBarBounds(VkRectangle::empty()), _scrollPosition(Vector2::zero()),
...
unsigned int Container::draw(Form* form, const VkRectangle& clip)
{
    if (!_visible)
        return 0;
    ...
    {
        // Draw scroll bars.
        VkRectangle clipRegion(_absoluteClipBounds);
        ...
        {
            const VkRectangle& topRegion = _scrollBarTopCap->getRegion();
            ...
            const VkRectangle& verticalRegion = _scrollBarVertical->getRegion();
            ...
            const VkRectangle& bottomRegion = _scrollBarBottomCap->getRegion();
            ...
            VkRectangle bounds(_viewportBounds.right() + (_absoluteBounds.right() - _viewportBounds.right())*0.5f - topRegion.width*0.5f, _viewportBounds.y + _scrollBarBounds.y, topRegion.width, topRegion.height);
        ...
        {
            const VkRectangle& leftRegion = _scrollBarLeftCap->getRegion();
            ...
            const VkRectangle& horizontalRegion = _scrollBarHorizontal->getRegion();
            ...
            const VkRectangle& rightRegion = _scrollBarRightCap->getRegion();
            ...
            VkRectangle bounds(_viewportBounds.x + _scrollBarBounds.x, _viewportBounds.bottom() + (_absoluteBounds.bottom() - _viewportBounds.bottom())*0.5f - leftRegion.height*0.5f, leftRegion.width, leftRegion.height);
...
bool Container::moveFocusDirectional(Direction direction)
{
	...
	const VkRectangle& startBounds = startControl->_absoluteBounds;
	...
	for (size_t i = 0, count = _controls.size(); i < count; ++i)
	{
		...
		const VkRectangle& nextBounds = ctrl->getAbsoluteBounds();
		switch (direction)
		{
		case UP:
...
void Container::updateScroll()
{
    ...
    for (size_t i = 0, count = controls.size(); i < count; ++i)
    {
        ...
        const VkRectangle& bounds = control->getBounds();
