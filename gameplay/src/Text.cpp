...
Text::Text() :
    ...
    _align(Font::ALIGN_TOP_LEFT), _clip(VkRectangle(0, 0, 0, 0)),
...
void Text::setClip(const VkRectangle& clip)
{
    _clip = clip;
}

const VkRectangle& Text::getClip() const
{
    return _clip;
}
...
unsigned int Text::draw(bool wireframe)
{
    ...
    VkRectangle viewport = Game::getInstance()->getViewport();
    ...
    VkRectangle clipViewport = _clip;
    ...
    _drawFont->start();
    _drawFont->drawText(_text.c_str(), VkRectangle(position.x, position.y, _width, _height),
