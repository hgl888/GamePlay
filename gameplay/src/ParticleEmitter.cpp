...
void ParticleEmitter::setTexture(Texture* texture, BlendMode blendMode)
{
    ...
    VkRectangle texCoord((float)texture->getWidth(), (float)texture->getHeight());
...
void ParticleEmitter::setSpriteFrameCoords(unsigned int frameCount, VkRectangle* frameCoords)
{
    GP_ASSERT(frameCount);
...
void ParticleEmitter::setSpriteFrameCoords(unsigned int frameCount, int width, int height)
{
    ...
    VkRectangle* frameCoords = new VkRectangle[frameCount];
    ...
    for (unsigned int i = 0; i < rows; ++i)
    {
        ...
        {
            int x = j * width;
            frameCoords[i*cols + j] = VkRectangle(x, y, width, height);
            if (++n == frameCount)
            {
                break;
            }
