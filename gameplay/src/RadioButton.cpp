...
void RadioButton::updateBounds()
{
    ...
    if (_selected)
    {
        const VkRectangle& selectedRegion = getImageRegion("selected", NORMAL);
        size.set(selectedRegion.width, selectedRegion.height);
    }
    else
    {
        const VkRectangle& unselectedRegion = getImageRegion("unselected", NORMAL);
...
unsigned int RadioButton::drawImages(Form* form, const VkRectangle& clip)
{
    if (!_image)
        return 0;
    ...
    const VkRectangle& region = _image->getRegion();
